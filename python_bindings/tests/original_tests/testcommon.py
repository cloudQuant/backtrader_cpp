...
flushfile = bt.utils.flushfile
ParamsBase = bt.metabase.ParamsBase
...
# Import feeds directly for compatibility
DATAFEED = bt.feeds.CSVDataFeed
...
def getdata(index, fromdate=FROMDATE, todate=TODATE):
    ...
    data = DATAFEED(datapath)  # Only pass filename for C++ compatibility

    # For C++ compatibility, create a DataSeries wrapper
    if hasattr(data, 'load_data'):
        data.load_data()  # Load the data
        # Create a DataSeries and copy data
        dataseries = bt.DataSeries("data")
        # For now, return the feed directly since inheritance should work
        return data
...
def runtest(datas,
    ...
    for prload in preloads:
        for ronce in runonces:
            for exbar in exbars:
                cerebro = bt.Cerebro()  # Simplified for backtrader_cpp compatibility

                if kwargs.get('main', False):
                    print('prload {} / ronce {} exbar {}'.format(
                        prload, ronce, exbar))

                if isinstance(datas, bt.metabase.LineSeries):
                    datas = [datas]
                for data in datas:
                    cerebro.add_data(data)

                if not optimize:
                    # Instantiate strategy class for C++ compatibility
                    # Create strategy with basic parameters
                    strategy_instance = strategy()
                    cerebro.add_strategy(strategy_instance)
...
class TestStrategy(bt.Strategy):
    ...
    def __init__(self):
        # Call parent __init__ for C++ compatibility
        super().__init__()

        # For C++ compatibility, simplify the initialization
        # The original test parameters will be set externally
        pass
