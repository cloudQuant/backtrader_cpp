...
double LineBuffer::operator[](int index) const {
    ...
    if (index >= 0) {
        // Past values: positive indices go backward from current
        ...
        // Future values: negative indices go forward from current
        int actual_idx = _idx - index;
...
void LineBuffer::set(int index, double value) {
    if (index == 0) {
        ...
        throw std::runtime_error("Setting non-current values not supported");
    }
}
