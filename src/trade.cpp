...
std::string TradeHistory::to_string() const {
    ...
    oss << "TradeHistory[";
    oss << "status=" << (status.status == TradeStatus::Open ? "Open" : "Closed");
...
Trade::Trade() : ref(next_ref_++), just_opened_(false) {
}
...
void Trade::update(std::shared_ptr<Order> order,
    ...
    // Update trade totals
    size += static_cast<int>(size_change);
    commission += commission_val;
    value += value_change;
    pnl = pnl_val;
    pnlcomm = pnl - commission;
    
    // Update pricing (weighted average for opening positions)
    if (size != 0) {
        if (status == TradeStatus::Open) {
            // For open trades, maintain weighted average price
            price = (price * (size - size_change) + price_val * size_change) / size;
        } else {
            // For new trades
            price = price_val;
            dtopen = dt;
...
std::string Trade::to_string() const {
    std::ostringstream oss;
    oss << "Trade[" << ref << "] ";
    oss << (status == TradeStatus::Open ? "OPEN" : "CLOSED") << " ";
    oss << "Size: " << size << " @ " << std::fixed << std::setprecision(4) << price;
    if (isclosed()) {
