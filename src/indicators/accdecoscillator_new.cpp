...
void AccelerationDecelerationOscillator::calculate() {
    ...
    std::cerr << "AccDecOsc::calculate - ENTRY POINT" << std::endl;
...
void AccelerationDecelerationOscillator::once(int start, int end) {
    ...
    auto ao = std::make_shared<AwesomeOscillator>(ao_data_source);
    ao->calculate();
    ...
    auto sma = std::make_shared<SMA>(ao_line_series, 5);
    sma->calculate();
