...
void KAMAOscillator::next() {
    ...
    // Get KAMA value
    double kama_value = kama1_->get(0);
    
    if (!std::isnan(data_value) && !std::isnan(kama_value)) {
        // Calculate oscillator as data - KAMA (not difference between two KAMAs)
        double osc_value = data_value - kama_value;
