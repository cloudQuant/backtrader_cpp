...
TEST(OriginalTests, MetaClass_IndicatorIntegration) {
    ...
    // 验证指标计算正常
    if (csv_data.size() >= 20) {
        double final_value = custom_sma->get(0);
        EXPECT_FALSE(std::isnan(final_value)) << "Final SMA value should not be NaN";
        EXPECT_TRUE(std::isfinite(final_value)) << "Final SMA value should be finite";
    }
}
