...
TEST(OriginalTests, PctChange_Manual) {
    ...
    auto pctchange = std::make_shared<PctChange>(close_line, 30);
...
TEST_P(PctChangeParameterizedTest, DifferentPeriods) {
    int period = GetParam();
    auto pctchange = std::make_shared<PctChange>(close_line_, period);
...
TEST(OriginalTests, PctChange_CalculationLogic) {
    ...
    auto pctchange = std::make_shared<PctChange>(close_line, 5);
    ...
    for (size_t i = 0; i < prices.size(); ++i) {
        ...
        if (i >= 5) {  // 需要period + 1个数据点
            ...
            double actual_pctchange = pctchange->get(0);
            EXPECT_NEAR(actual_pctchange, expected_pctchange, 1e-10) 
                << "PctChange calculation mismatch at step " << i 
...
TEST(OriginalTests, PctChange_vs_ROC) {
    ...
    for (const auto& bar : csv_data) {
        ...
        close_line_roc->forward(bar.close);
    }
    
    auto pctchange = std::make_shared<PctChange>(close_line_pct, 12);
    auto roc = std::make_shared<ROC>(close_line_roc, 12);
    ...
    for (size_t i = 0; i < csv_data.size(); ++i) {
        ...
        if (i < csv_data.size() - 1) {
            close_line_pct->forward();
            close_line_roc->forward();
        }
    }
}
...
TEST(OriginalTests, PctChange_TrendDetection) {
    ...
    auto up_pctchange = std::make_shared<PctChange>(up_line, 10);
    ...
    auto down_pctchange = std::make_shared<PctChange>(down_line, 10);
...
TEST(OriginalTests, PctChange_PercentageCalculation) {
    ...
    auto pctchange = std::make_shared<PctChange>(close_line, 3);
    ...
    for (size_t i = 0; i < prices.size(); ++i) {
        ...
        if (i >= 3) {
            ...
            double actual_pct = pctchange->get(0);
            
            EXPECT_NEAR(actual_pct, expected_pct, 1e-10) 
                << "PctChange percentage calculation at step " << i;
...
TEST(OriginalTests, PctChange_SidewaysMarket) {
    ...
    auto sideways_pctchange = std::make_shared<PctChange>(sideways_line, 20);
...
TEST(OriginalTests, PctChange_ZeroCrossing) {
    ...
    auto pctchange = std::make_shared<PctChange>(close_line, 12);
...
TEST(OriginalTests, PctChange_EdgeCases) {
    ...
    auto zero_pctchange = std::make_shared<PctChange>(zero_line, 3);
    ...
    auto flat_pctchange = std::make_shared<PctChange>(flat_line, 10);
...
TEST(OriginalTests, PctChange_HighVolatility) {
    ...
    auto volatile_pctchange = std::make_shared<PctChange>(volatile_line, 10);
...
TEST(OriginalTests, PctChange_Performance) {
    ...
    auto large_pctchange = std::make_shared<PctChange>(large_line, 50);
