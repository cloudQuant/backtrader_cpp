...
TEST(OriginalTests, LRSI_CalculationLogic) {
    ...
    auto price_line = std::make_shared<LineRoot>(prices.size(), "lrsi_calc");
...
TEST(OriginalTests, LRSI_TrendFollowing) {
    ...
    auto trend_line = std::make_shared<LineRoot>(uptrend_prices.size(), "trend");
...
TEST(OriginalTests, LRSI_EdgeCases) {
    ...
    // 测试数据不足的情况
    auto insufficient_line = std::make_shared<LineRoot>(10, "insufficient");
