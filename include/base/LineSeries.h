...
public:
     ...
     */
    auto getLineRange() const {
        return lines_ | std::views::filter([](const auto& line) { return line != nullptr; });
    }
     ...
     */
    std::optional<std::span<const double>> getDataSpan(const std::string& line_name, int periods) const;
