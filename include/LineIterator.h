...
public:
    ...
    std::vector<std::shared_ptr<LineBuffer>> getDataRange() const {
        std::vector<std::shared_ptr<LineBuffer>> result;
        std::copy_if(data_buffers_.begin(), data_buffers_.end(), std::back_inserter(result),
                     [](const auto& buf) { return buf != nullptr; });
        return result;
    }
